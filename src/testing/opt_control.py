...
def vehicle_update(t, x, u, params):
    ...
    return np.array([
        ...
        phi#(u[0] / l) * np.tan(phi)        # thdot = v/l tan(phi)
    ])
...
xf = np.array([5., 5., math.pi/2]); uf = np.array([0., 0.])
Tf = 5
...
constraints = [ opt.input_range_constraint(vehicle, [0, -3], [2, 3])]#, test_constraint]#, test_constraint]#, lp_constraint ]
