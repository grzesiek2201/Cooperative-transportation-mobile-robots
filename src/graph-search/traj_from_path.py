    ...
    for i in range(1, len(pts)):
        ...
        if pts[i][0] != traj.item[-1][0][0] or pts[i][1] != traj.item[-1][0][1]:  # linear or linear and angular movement
            ...
            if v_decel:     # linear deceleration
                ...
                t += abs((vk - traj.get_last_vel()) / a)   # length of this time step # t = (vk - v0)/a
            ...
            if w_decel:
                ...
                wk = math.copysign(wk, ((pts[i][2] + math.pi) % (2*math.pi) - math.pi) - ((traj.item[-1][0][2] + math.pi) % (2*math.pi) - math.pi))
                t += abs((wk - traj.get_last_rot()) / e)   # length of this time step # t = (vk - v0) / a
                
            else:   # constant speed
                t += s / wmax   # length of this time step
                wk = wmax
                wk = math.copysign(wk, ((pts[i][2] + math.pi) % (2*math.pi) - math.pi) - ((traj.item[-1][0][2] + math.pi) % (2*math.pi) - math.pi))
...
def traj_to_csv(traj, filename='trajectory.csv'):
    ...
    path = list(Path(__file__).parent.parent.glob(f"graph-search/{filename}"))[0]
            ...
            for i in range(len(segment)):
                ...
                if ori_dif == 0:    #  straight movement, no rotation
                    ...
                    for x, y, theta in intr_states:
                        ...
                        # thetas.append(theta)
                    intr_states = [(xy[0]+x0, xy[1]+y0, theta) for xy, theta in zip(intr_states, thetas)]
                    states += intr_states[:-1]
